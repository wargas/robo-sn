...
export class CrawlerPagamento extends Work {
    ...
    async process(inscricao: string) {
        ...
        try {
            ...
            await paginate.forEach(async() => {
                ...
                items.push(...data.map((d: any) => {
                    return {
                        ...
                        competencia: get(d, 'Competência', '').replace(/(\d{2})\/(\d{4})/, "$2-$1"),
                        devido: toNumber(get(d, 'ISSDevido', '0')),
                        recolhido: toNumber(get(d, 'ISSRecolhido', '0')),
                        parcelado: toNumber(get(d, 'ISSParcelado', '0')),
            ...
            this.events.emit('fail', inscricao)
            return []
        }
